...
const statusOptions = ["ACTIVE", "INACTIVE", "HARVESTED"]; // Add all backend CropStatus enums
...
const [irrigationNeeds, setIrrigationNeeds] = useState("");
  ...
  try {
    const response = await generateGeminiContent(prompt.trim());
    // Example parsing - you can adjust according to AI response format
  ...
  if (!plantName || !category || !plantingDate || !area || area <= 0) {
    toast({
      title: "Invalid Input",
  ...
  if (!plotId || !farmId) {
    toast({
      title: "Missing Data",
      description: "Plot or farm information is missing.",
      variant: "destructive",
    });
    return;
  }
